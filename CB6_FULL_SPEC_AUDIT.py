...
for token in ('NEAR_REFRESH_MS = 90 * 1000L','NEAR_RETRY_MS = 20 * 1000L','NEAR_REFRESH_DISTANCE_M = 500.0f','NEAR_SIGNAL_RADIUS_M = 2000','requestNearbySignalsIfNeeded(lat, lon, now);','postNearbySignals(nearby, lat, lon);','mLastPublishedPoints','overpass.nchc.org.tw'):check(token in mgr,'signal acquisition resilience: '+token)
...
for token in ('class Cb6SignalMark final : public DebugMarkPoint','DebugMarkPoint(pt, UserMark::Type::CB6_DRIVING)','GetMinZoom() const override { return 15; }','symbols->insert({17, "cb6-signal-m"});','symbols->insert({19, "cb6-signal-l"});','IsNonDisplaceable() const override { return true; }','GetDepthTestEnabled() const override { return false; }','session.ClearGroup(UserMark::Type::CB6_DRIVING);','session.SetIsVisible(UserMark::Type::CB6_DRIVING, true);','session.CreateUserMark<Cb6SignalMark>(pt)','session.NotifyChanges();'):check(token in fwcpp,'signal renderer: '+token)
