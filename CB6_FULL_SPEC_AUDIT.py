...
for token in ('class Cb6SignalMark final : public DebugMarkPoint','DebugMarkPoint(pt, UserMark::Type::CB6_DRIVING)','GetMinZoom() const override { return 14; }','symbols->insert({14, "cb6-signal-500"});','symbols->insert({15, "cb6-signal"});','symbols->insert({17, "cb6-signal-m"});','symbols->insert({19, "cb6-signal-l"});','IsNonDisplaceable() const override { return true; }','GetDepthTestEnabled() const override { return false; }','session.ClearGroup(UserMark::Type::CB6_DRIVING);','session.SetIsVisible(UserMark::Type::CB6_DRIVING, true);','session.CreateUserMark<Cb6SignalMark>(pt)','session.NotifyChanges();'):check(token in fwcpp,'signal renderer: '+token)
