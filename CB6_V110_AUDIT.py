...
for token,label in (('class Cb6SignalMark final : public DebugMarkPoint','DebugMarkPoint signal path'),('GetMinZoom() const override { return 14; }','z14 minimum for 500m signal visibility'),('symbols->insert({14, "cb6-signal-500"});','500m half-size signal icon'),('symbols->insert({15, "cb6-signal"});','standard signal icon from z15'),('symbols->insert({17, "cb6-signal-m"});','z17 forward icon'),('symbols->insert({19, "cb6-signal-l"});','z19 forward icon'),('IsNonDisplaceable() const override { return true; }','non-displaceable'),('GetDepthTestEnabled() const override { return false; }','depth disabled')): need(fs,token,label)
