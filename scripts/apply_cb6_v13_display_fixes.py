...
# Address policy: suppress fine suburb/locality/quarter/neighbourhood labels through z16,
# then allow them again at z17+ (roughly the 100 m detail range on CB6).
for p in sorted((ROOT / "data/styles").glob("*/include/Basemap_label.mapcss")):
    ...
    # v1.1 rule must also stop at z16, otherwise it would continue hiding labels at z17+.
...
# Signals + convenience marks: start at a wider zoom. Existing small/large symbol switch
# still makes icons grow as the user zooms in, avoiding large icons at overview scale.
...
# Native signal fallback in all map styles also starts earlier.
...
# Bottom UI: compact road HUD in the safe center strip. Google Maps button is stacked
# above the bottom-right controls instead of sharing the HUD row.
rel="android/app/src/main/java/app/organicmaps/MwmActivity.java"
s=read(rel)
s=once(s,
'''      lp.width = cb6Dp(landscape ? 170 : 210);\n      lp.height = cb6Dp(32);''',
'''      lp.width = cb6Dp(landscape ? 150 : 190);\n      lp.height = cb6Dp(28);''',"road HUD compact size")
...
# second occurrence belongs to Google button: move it upward above the right-bottom control.
