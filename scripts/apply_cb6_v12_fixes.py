...
def write(rel,s):
    p=ROOT/rel; p.write_text(s,encoding="utf-8"); print("v1.2 patched:",rel)
...
body='''\n  auto const point = mercator::FromLatLon(lat, lon);
  ...
  {
    if (ft.GetGeomType() != feature::GeomType::Line) return;
    bool isRoad = false;
    ft.ForEachType([&](uint32_t type) { auto const typeName = classif().GetReadableObjectName(type); if (typeName.rfind("highway-", 0) == 0) isRoad = true; });
    if (!isRoad) return;
    std::string name(ft.GetName(localisation::kDefaultNameIndex));
    if (name.empty()) name = ft.GetTranslatedName().m_primary.value_or(std::string());
...
for p in sorted((ROOT/"data/styles").glob("*/include/Icons.mapcss")):
    x=p.read_text(encoding="utf-8"); x=re.sub(r"node\|z(?:1[0-8])-(\[amenity=parking\])",r"node|z19-\1",x)
    if "CB6 v1.2: reduce individual parking icon density" not in x: x+='''\n\n/* CB6 v1.2: reduce individual parking icon density at normal driving zooms. */
node|z19-[amenity=parking] {icon-min-distance: 60;}
'''
    p.write_text(x,encoding="utf-8")
print("CB6 Navi v1.2 fixes applied.")
