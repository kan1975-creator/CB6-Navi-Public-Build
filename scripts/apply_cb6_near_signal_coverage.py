...
s = s.replace('private static final long NEAR_REFRESH_MS = 90 * 1000L;',
              'private static final long NEAR_REFRESH_MS = 45 * 1000L;', 1)
s = s.replace('private static final long NEAR_RETRY_MS = 20 * 1000L;',
              'private static final long NEAR_RETRY_MS = 12 * 1000L;', 1)
s = s.replace('private static final float NEAR_REFRESH_DISTANCE_M = 500.0f;',
              'private static final float NEAR_REFRESH_DISTANCE_M = 250.0f;', 1)
s = s.replace('private static final int NEAR_SIGNAL_RADIUS_M = 2000;',
              'private static final int NEAR_SIGNAL_RADIUS_M = 3000;', 1)

# Query nodes explicitly and ask Overpass for geometry/body. This remains
# highway=traffic_signals only; no speculative intersection synthesis.
s = s.replace('+ ");out body;";', '+ ");out body qt;";', 1)
