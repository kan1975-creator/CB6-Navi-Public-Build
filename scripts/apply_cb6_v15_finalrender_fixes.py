    ...
    for line in lines:
        if not in_block:
            ...
            if "{" in line:
                ...
                if "}" in line:
                    text = "".join(block)
                    if "[highway=traffic_signals]" not in text:
                        out.extend(block)
                    block = []
                    in_block = False
...
if "mCb6SupplementManager.reapplySavedMarks();" not in s:
    ...
    if restart_anchor not in s:
        raise SystemExit("ThemeSwitcher restart anchor missing")
...
if "private void refreshCb6FromSavedLocation()" not in s:
    if helper_anchor not in s:
        raise SystemExit("road HUD helper anchor missing")
...
for forbidden in ("cb6-fuel", "cb6-hospital", "cb6-station", "cb6-supermarket", "cb6-mall", "cb6-airport"):
    if forbidden in us:
        raise SystemExit("standard landmark leaked into CB6 UserMarks: " + forbidden)
...
for p in signal_style_files:
    ...
    after = strip_native_signal_rules(before)
    if after != before:
...
for style, block in low_zoom_rules.items():
    ...
    if not p.exists():
        raise SystemExit("missing native landmark style: " + str(p.relative_to(ROOT)))
    text = p.read_text(encoding="utf-8")
    marker = "CB6 v1.5: native major-landmark low-zoom bridge"
    if marker not in text:
        p.write_text(text + block, encoding="utf-8")
        print("v1.5 native landmarks:", p.relative_to(ROOT))
