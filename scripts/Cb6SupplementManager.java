...
public final class Cb6SupplementManager
{
  ...
  public Cb6SupplementManager(@NonNull Context context)
  {
  ...
  {
    // Keep one synthetic signal about 120 m north of the current position. It is
    // merged into every CACHE/NET/native update so later ClearGroup() calls can no
    // longer erase the diagnostic before it reaches the renderer.
  ...
  private void applySavedCache()
  {
    ...
    try
    {
      Points points = parseCached(new JSONArray(json));
      postPoints(points, "CACHE");
      Log.i(TAG, "reapplied " + points.size() + " cached points, signals=" + points.signalCount());
    }
    catch (Exception e)
    {
      Log.w(TAG, "cache parse failed", e);
      showStatus("SIG CACHE: 読込失敗");
      postDiagnosticOnly();
    }
  }
  ...
  private void refresh(double lat, double lon)
  {
    ...
    for (String endpoint : endpoints)
    {
      try
      {
        ...
        mContext.getSharedPreferences(PREF, Context.MODE_PRIVATE).edit()
                .putString(CACHE, points.toJson().toString())
                .putLong(CACHE_TIME, System.currentTimeMillis())
                .apply();
        postPoints(points, "NET");
        Log.i(TAG, "loaded " + points.size() + " supplemental points, signals=" + points.signalCount());
        return;
      }
      catch (Exception e)
      {
    ...
    showStatus("SIG NET: 取得失敗 " + lastError);
  ...
  private void postDiagnosticOnly()
  {
    if (Double.isNaN(mDiagnosticLat) || Double.isNaN(mDiagnosticLon))
      return;
    mMain.post(() -> {
      Framework.nativeSetCb6DrivingMarks(
          new double[] { mDiagnosticLat }, new double[] { mDiagnosticLon },
          new int[] { KIND_SIGNAL_FULL });
      showStatus("SIG TEST: 強制信号1件をJNIへ維持");
      Log.i(TAG, "persistent diagnostic signal at " + mDiagnosticLat + "," + mDiagnosticLon);
    });
  }
  ...
  {
    mMain.post(() -> Toast.makeText(mContext, text, Toast.LENGTH_LONG).show());
  }

  private static String buildQuery(double lat, double lon)
  {
    String ll = String.format(Locale.US, "%.6f,%.6f", lat, lon);
    return "[out:json][timeout:25];("
        + "node(around:5000," + ll + ")[highway=traffic_signals];"
        + "nwr(around:6500," + ll + ")[shop=convenience];"
        + "node(around:3500," + ll + ")[highway=stop];"
        + ");out center tags;";
  }
  ...
  private static String post(String endpoint, String query) throws Exception
  {
    ...
    c.setDoOutput(true);
    c.setRequestProperty("User-Agent", "CB6-Navi/SignalDiagnostic CoMaps-supplement");
  ...
  private static Points parseOverpass(JSONObject root) throws Exception
  {
    ...
    int signalsAdded = 0;
    for (int i = 0; i < elements.length() && signalsAdded < MAX_SIGNAL_POINTS; ++i)
    {
    ...
    for (int i = 0; i < elements.length() && out.size() < MAX_POINTS; ++i)
    {
      JSONObject e = elements.optJSONObject(i);
      if (e == null)
        continue;
      JSONObject tags = e.optJSONObject("tags");
      if (tags == null || !"convenience".equals(tags.optString("shop", "")))
        continue;
      double[] pos = positionOf(e);
      if (pos == null)
        continue;
      out.add(pos[0], pos[1], classifyConvenience(tags));
    }
    ...
    for (int i = 0; i < elements.length() && out.size() < MAX_POINTS; ++i)
    {
      ...
      if (e == null)
        continue;
      JSONObject tags = e.optJSONObject("tags");
      if (tags == null || !"stop".equals(tags.optString("highway", "")))
        continue;
      double[] pos = positionOf(e);
      if (pos != null)
        out.add(pos[0], pos[1], 0);
    }

    return out;
  }

  private static double[] positionOf(JSONObject e)
  {
    if (e.has("lat") && e.has("lon"))
      return new double[] {e.optDouble("lat"), e.optDouble("lon")};
    JSONObject center = e.optJSONObject("center");
    if (center != null && center.has("lat") && center.has("lon"))
      return new double[] {center.optDouble("lat"), center.optDouble("lon")};
    return null;
  }

  private static int classifyConvenience(JSONObject tags)
  {
    String source = (tags.optString("brand", "") + " " + tags.optString("name", "") + " "
        + tags.optString("operator", "")).toLowerCase(Locale.ROOT);
    String n = source.replace(" ", "").replace("-", "").replace("‐", "")
                     .replace("‑", "").replace("–", "").replace("—", "");
    if (n.contains("7eleven") || n.contains("セブンイレブン")) return 1;
    if (n.contains("familymart") || n.contains("ファミリーマート") || n.contains("ファミマ")) return 2;
    if (n.contains("lawson") || n.contains("ローソン")) return 3;
    if (n.contains("seicomart") || n.contains("セイコーマート") || n.contains("セコマ")) return 4;
    if (n.contains("mybasket") || n.contains("まいばすけっと")) return 5;
    if (n.contains("ministop") || n.contains("ミニストップ")) return 8;
    if (n.contains("dailyyamazaki") || n.contains("デイリーヤマザキ") || n.contains("yamazakidaily")) return 9;
    return 6;
  }

  private static Points parseCached(JSONArray a) throws Exception
  {
    Points out = new Points();
    for (int i = 0; i < a.length() && out.size() < MAX_POINTS; ++i)
    {
      JSONArray p = a.getJSONArray(i);
      int kind = p.getInt(2);
      if (!((kind >= 0 && kind <= 9) || kind == KIND_SIGNAL_CLUSTER))
        continue;
      out.add(p.getDouble(0), p.getDouble(1), kind);
    }
    return out;
  }
  ...
  private static final class Points
  {
    ...
    private final ArrayList<Integer> kind = new ArrayList<>();

    void add(double a, double o, int k)
    {
      lat.add(a); lon.add(o); kind.add(k);
    }
    ...
    double[] lons()
    {
      double[] a = new double[lon.size()];
      for (int i = 0; i < a.length; ++i) a[i] = lon.get(i);
      return a;
    }

    int[] kinds()
    {
      int[] a = new int[kind.size()];
      for (int i = 0; i < a.length; ++i) a[i] = kind.get(i);
      return a;
    }
    ...
    JSONArray toJson()
    {
      ...
      for (int i = 0; i < size(); ++i)
      {
        ...
        p.put(lat.get(i)); p.put(lon.get(i)); p.put(kind.get(i));
        a.put(p);
      }
      return a;
    }
